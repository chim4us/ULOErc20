...
contract casino is ownable, Destroyable{
    ...
    function Join(address _ref,uint _JoinCst) public payable checkRef(_ref) cost(JoinCst, msg.sender){
        ...
        AddBal(CRef,CjoinCst);
        TotalSupply += CjoinCst;
        assert(Token.balanceOf(address(this)) >= TotalSupply);
        emit JoinedEvt (msg.sender, MembCount, Creator[msg.sender].ref);
        MembCount += 1;
    }
    
    function updateContrctFee(uint _JoinCst,uint _RefPercnt) public onlyOwner{
        JoinCst = _JoinCst;
        RefPercnt = _RefPercnt;
    }
    ...
    function ContractDetials() public view onlyOwner returns(uint Bal,uint TokenBal,uint _JoinCst, uint _RefPercnt){
        return (address(this).balance,Token.balanceOf(address(this)),JoinCst,RefPercnt);
    }
